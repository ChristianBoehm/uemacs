...
char *gtfun(char *fname)
{
	...
	switch (fnum) {
	case UFADD:
		return _itoa(atoi(arg1) + atoi(arg2));
	case UFSUB:
		return _itoa(atoi(arg1) - atoi(arg2));
	case UFTIMES:
		return _itoa(atoi(arg1) * atoi(arg2));
	...
	case UFGTKEY:
		...
		return result;
	case UFRND:
		return _itoa((ernd() % abs(atoi(arg1))) + 1);
	case UFABS:
		return _itoa(abs(atoi(arg1)));
	case UFSINDEX:
		return _itoa(sindex(arg1, arg2));
	case UFENV:
#if	ENVFUNC
		tsp = getenv(arg1);
		return tsp == NULL ? "" : tsp;
...
char *gtenv(char *vname)
{
	...
	switch (vnum) {
	case EVFILLCOL:
		return _itoa(fillcol);
	case EVPAGELEN:
		return _itoa(term.t_nrow + 1);
	case EVCURCOL:
		return _itoa(getccol(FALSE));
	case EVCURLINE:
		return _itoa(getcline());
	case EVRAM:
		return _itoa((int) (envram / 1024l));
	case EVFLICKER:
		return ltos(flickcode);
	case EVCURWIDTH:
		return _itoa(term.t_ncol);
	case EVCBUFNAME:
	...
	case EVASAVE:
		return _itoa(gasave);
	case EVACOUNT:
		return _itoa(gacount);
	case EVLASTKEY:
		return _itoa(lastkey);
	case EVCURCHAR:
		return (curwp->w_dotp->l_used ==
			curwp->w_doto ? _itoa('\n') :
			_itoa(lgetc(curwp->w_dotp, curwp->w_doto)));
	case EVDISCMD:
		return ltos(discmd);
	case EVVERSION:
		return VERSION;
	...
	case EVDISINP:
		return ltos(disinp);
	case EVWLINE:
		return _itoa(curwp->w_ntrows);
	case EVCWLINE:
		return _itoa(getwpos());
	case EVTARGET:
		saveflag = lastflag;
		return _itoa(curgoal);
	case EVSEARCH:
		return pat;
	case EVREPLACE:
		return rpat;
	...
	case EVKILL:
		return getkill();
	case EVCMODE:
		return _itoa(curbp->b_mode);
	case EVGMODE:
		return _itoa(gmode);
	case EVTPAUSE:
		return _itoa(term.t_pause);
	case EVPENDING:
#if	TYPEAH
...
#endif
	case EVLWIDTH:
		return _itoa(llength(curwp->w_dotp));
	case EVLINE:
		return getctext();
	case EVGFLAGS:
		return _itoa(gflags);
	case EVRVAL:
		return _itoa(rval);
	case EVTAB:
		return _itoa(tabmask + 1);
	case EVOVERLAP:
		return _itoa(overlap);
	case EVSCROLLCOUNT:
		return _itoa(scrollcount);
#if SCROLLCODE
	case EVSCROLL:
...
/*
 * _itoa:
 *  before function name itoa(int i) renamed to 
 *  _itoa(int i) to mark it as internal. Before it 
 *  was conflicting CYGWIN's itoa func.
 * 
